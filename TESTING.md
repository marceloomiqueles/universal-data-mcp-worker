...
Coverage includes exact decimal-string/`BigInt` money handling, currency and IANA-timezone validation, current order and line-item updates, cancellation exclusion, nullable catalog references, nested and top-level pagination, bounded continuation, concurrency rejection, non-destructive partial failures, deterministic upserts, exact sales totals/averages, merchant calendar boundaries, and successful zero-order results. The authenticated `POST /api/integrations/shopify/orders/sync` endpoint returns only bounded operational metadata. The D1 sales read model is consumed by `get_sales`; no order reporting UI is included.

The 2026-08-16 live harness attempt stopped safely at connection verification with `SCOPE_FAILED` because the configured development-store app did not match the required order scopes. No order request or store mutation occurred. See [the sanitized backend validation record](docs/audits/2026-08-16-shopify-orders-ingestion-backend-validation.md). Rerun `pnpm test:shopify-live` after updating the maintainer-controlled app to the exact five-scope set, including `read_orders` and `read_all_orders`. Order acquisition remains explicitly bounded despite the broader history permission.
...
Admin SPA tests exercise the production Shopify order-sync client contract and operational UI states: required order-scope failure, never synchronized, duplicate-submission prevention while syncing, complete counts and coverage, partial coverage, failed synchronization with prior data preservation, and disconnected access. The UI exposes no customer data or sales dashboard. It states that `read_all_orders` permits older source access while the current ingestion implementation remains bounded to a recent 60-day window.
