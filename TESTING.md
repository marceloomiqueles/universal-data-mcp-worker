...
The production deployment-gate tests invoke Wrangler against isolated local D1 state. They prove that an existing database with a pending repository migration applies it before publication is allowed, a database with no pending migrations permits a repeat deployment, and a controlled invalid pending migration prevents the publication callback from running. These tests exercise the repository command boundary without contacting Cloudflare. On 2026-08-16, a real Workers Build verified root `/`, `pnpm build`, `pnpm deploy`, sufficient D1 permission, a successful remote no-pending migration check, and exactly one subsequent Worker deployment. A real remote pending or failing migration was deliberately not manufactured against production.
...
It requires non-empty `SHOPIFY_SHOP_DOMAIN`, `SHOPIFY_CLIENT_ID`, and `SHOPIFY_CLIENT_SECRET` values in ignored `.dev.vars`. The product requires `read_products`, `read_inventory`, `read_locations`, and `read_orders`; verification accepts any provider-reported grant containing that required subset. `read_all_orders` is not required and does not expand the product's recent 60-day order window if Shopify reports it as an additional granted scope. The harness creates an ephemeral owner and encryption key, records required and provider-reported scopes separately, saves configuration through the authenticated production endpoint, verifies the real development store, invokes the production inventory and order-sync endpoints, and asserts encrypted-at-rest configuration, complete bounded coverage, stable inventory row counts, absence of customer-identity schema, and absence of access-token persistence. It performs no mutation and prints only a sanitized summary. The command is excluded from ordinary `pnpm test`; never use it with a merchant or production store.
...
## Shopify orders ingestion backend

Worker tests apply migration `0004_shopify_orders.sql` and exercise the production Shopify order-sync path. The source query is read-only, requests no customer identity fields, uses the fixed `2026-07` API, and covers a fixed recent 60-day window with 20 orders per page, 20 line items per nested page, at most five completed order pages, and at most 20 GraphQL requests per invocation. Normalized D1 checkpoint tables allow nested line-item pagination to resume without exposing provider cursors through the Admin API.

Coverage includes exact decimal-string/`BigInt` money handling, currency and IANA-timezone validation, current order and line-item updates, cancellation exclusion, nullable catalog references, nested and top-level pagination, bounded continuation, concurrency rejection, non-destructive partial failures, deterministic upserts, exact sales totals/averages, merchant calendar boundaries, and successful zero-order results. The mutable order tables retain the latest valid entity state, while run metadata separately identifies the latest attempt and latest complete coverage. Tests prove that a later failed or partial attempt preserves query availability from prior trustworthy coverage with conservative freshness, and that partial-only/failed-only state remains unavailable. The authenticated `POST /api/integrations/shopify/orders/sync` endpoint returns only bounded operational metadata.

The initial 2026-08-16 live harness attempt stopped safely at connection verification with `SCOPE_FAILED` before `read_orders` was available. That historical record remains unchanged. Current authoritative behavior requires the four-scope subset above, tolerates additional provider-reported scopes, and keeps order acquisition explicitly bounded. See the later Shopify sales validation and remediation records for superseding live evidence.

## Shopify sales MCP

Production-path MCP tests discover `get_sales` only with connected Shopify, invoke it through the real OAuth-protected MCP composition, and verify exact D1-backed totals, average order value, bounded top-product output, freshness, recent-60-day coverage, and successful zero-order periods. The same test intercepts global `fetch` and proves that both `get_inventory` and `get_sales` complete without Shopify network access. Zod input validation enforces either a merchant-timezone period or both explicit UTC endpoints, a maximum top-product limit of 25, and no extra fields.

Admin SPA tests exercise the production Shopify order-sync client contract and operational UI states: required order-scope failure, never synchronized, duplicate-submission prevention while syncing, complete counts and coverage, partial coverage, failed synchronization with prior data preservation, and disconnected access. The UI exposes no customer data or sales dashboard. It states that `read_orders` is required, additional granted permissions do not expand product coverage, and latest-attempt status remains distinct from the last successful complete order sync.

Migration `0004_shopify_orders.sql` and the Worker containing `get_sales` were deployed through the migration-before-code gate on 2026-08-16. Real ChatGPT Pro developer-mode validation subsequently covered both a complete zero-order sync and a positive development-store sync. Natural prompts invoked `get_sales`, returned one non-cancelled order totaling `CLP 3,330`, ranked two real line-item snapshots totaling the same amount, and preserved `America/Santiago` merchant-timezone context. The sanitized evidence is recorded in [Shopify Sales MCP Validation](docs/audits/2026-08-16-shopify-sales-mcp-validation.md).

The targeted remediation subsequently validated required-scope subset semantics against Shopify's actual client-credentials response, prior-complete sales availability after later incomplete attempts, and a real comparative ChatGPT prompt. The comparison performed the bounded current-week and prior-week sales evaluations in `America/Santiago`, matched read-only D1 evidence (`CLP 3,330`/1 order versus `CLP 0`/0 orders), and handled the zero baseline without inventing a percentage. See [Shopify Orders and Sales Remediation Validation](docs/audits/2026-08-16-shopify-orders-sales-remediation-validation.md).
