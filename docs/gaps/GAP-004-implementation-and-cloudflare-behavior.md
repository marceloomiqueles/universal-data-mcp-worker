...
The first MCP consumer selected `@modelcontextprotocol/sdk` 1.30.0 and its Web-standard Streamable HTTP transport after workerd compatibility validation. It also selected `@cloudflare/workers-oauth-provider` 0.10.3 for standard OAuth 2.1/PKCE token lifecycle in one Worker and one KV namespace. Local production-path tests validate both libraries without `nodejs_compat`. Deployed Worker, MCP Inspector, and real ChatGPT behavior remain evidence gaps.
