...
The repository cannot declare these trigger fields in `wrangler.jsonc`, so the effective dashboard values must be checked after initial installation and whenever the Git connection or build token changes. On 2026-08-16, a real Workers Build confirmed `pnpm build`, then `pnpm deploy`, then a successful remote no-pending migration check, followed by exactly one Worker deployment. This also confirmed that the selected build token could access D1. Pending and failing migration behavior remains validated against isolated D1 state; do not create a dummy production migration to repeat that proof remotely.
