...
The project has a deployable single-owner Admin application, a compiled integration registry, and a read-only MCP server. One Cloudflare Worker serves the Vue/Vuetify Admin SPA, authenticated Admin API, and an OAuth-protected MCP 2025-11-25 Streamable HTTP endpoint. Garmin remains registered but not configured. Shopify can be configured, verified, and synchronized on demand through the Admin Web. Its bounded catalog, inventory, and recent-order data is persisted in D1 and exposed through the D1-only `get_inventory` and `get_sales` tools alongside `list_integrations`. Real Cloudflare deployments and ChatGPT Work have validated the inventory and sales paths.

The current implementation preserves the single-deployment build and routing model without introducing a generic integration, commerce, or analytics framework.
...
| `DB` | Stores owner/session state and Shopify encrypted configuration, synchronized data, and operational metadata | D1 binding, not a secret | Required | Declared in `wrangler.jsonc`; local state persists under ignored `.wrangler/` | Bind to the deployment's real D1 database |
...
The current Shopify product requires `read_products`, `read_inventory`, `read_locations`, and `read_orders`. Verification accepts any provider-reported grant containing those scopes; additional granted scopes are not product requirements. In particular, `read_all_orders` is neither required nor consumed. Order ingestion remains deliberately bounded to the recent 60-day window even if Shopify reports broader access for a particular installation.
...
`get_sales` also reads D1 only. It accepts either an explicit UTC start/end range or one merchant-timezone period (`today`, `yesterday`, `this_week`, `last_week`, or `last_7_days`), with a maximum 60-day range and at most 25 optional top-product results. Sales is the sum of current non-cancelled Shopify order totals after returns, including taxes and discounts; it is not accounting revenue. Top-product amounts are current merchandise sales after discounts and before tax. Results include currency, average order value, the last successful complete order sync, the latest sync-attempt outcome, and the standard recent-60-day coverage limitation. A later failed or partial attempt does not hide previously complete coverage; freshness remains anchored to the last complete run. Real ChatGPT validation on 2026-08-16 covered truthful zero-order behavior, a positive `CLP 3,330` development-store order with two ranked line-item snapshots, and a two-period week comparison that correctly treated the prior week's zero baseline. Sanitized remediation evidence is recorded in [Shopify Orders and Sales Remediation Validation](docs/audits/2026-08-16-shopify-orders-sales-remediation-validation.md).
...
On the same date, the deployed Worker synchronized 18 real development-store products, 27 variants, 29 inventory levels, and 3 locations into D1, then ChatGPT discovered and naturally invoked `get_inventory`. It correctly answered out-of-stock, five-or-fewer, exact-SKU, and SKU-location questions from persisted data and reported the sync timestamp. Current validation covers the three read-only tools (`list_integrations`, `get_inventory`, and `get_sales`) in ChatGPT Work developer mode; it does not claim Garmin data tools, write actions, other ChatGPT plans, or other MCP clients. Sanitized inventory evidence is recorded in [the Shopify inventory MCP validation](docs/audits/2026-08-16-shopify-inventory-mcp-validation.md).
