...
| `INTEGRATION_SECRETS_KEY` | Encrypts persisted integration credentials with AES-256-GCM | Secret | Required for Shopify configuration | Generated and preserved in ignored `.dev.vars` by `pnpm setup:local` | Generated once by `provision:cloudflare` when absent; loss makes stored integration credentials unrecoverable, so never rotate it casually |
